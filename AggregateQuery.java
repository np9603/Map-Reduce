...
public class AggregateQuery {
		...
		public void map(LongWritable key, Text value, Context context)
			...
			if (key.get() != 0) {
				...
				if (whereResult) {
					...
					int aggregateValue = Integer.parseInt(columns[col_index
							.get(conf.get("aggregateTerm"))]);
					IntWritable outputValue = new IntWritable(aggregateValue);
	...
	public static class AggregateReducer extends
		...
		public void reduce(Text key, Iterable<IntWritable> values,
			...
			if (conf.getBoolean("isMax", false)) {
				result.set(max);
			} else if (conf.getBoolean("isMin", false)) {
				result.set(min);
			} else if (conf.getBoolean("isSum", false)) {
				result.set(sum);
			} else {
				result.set(1);
			}
	...
	public static void main(String[] args) throws Exception {
		...
		String aggregateTerm = SQL.substring(SQL.indexOf('(') + 1,
				SQL.indexOf(')'));
		System.out.println(aggregateTerm);
		conf.set("aggregateTerm", aggregateTerm);
		if (SQL.contains("max")) {
			conf.setBoolean("isMax", true);
		} else if (SQL.contains("min")) {
			conf.setBoolean("isMin", true);
		} else if (SQL.contains("sum")) {
			conf.setBoolean("isSum", true);
		}
